...
fs.readFile('config.json', (err, json) => {
  ...
  console.log('本次测试使用如下配置：')
  console.log(`  测试时间：${duration}分钟 （该设置尚未生效）`)
  console.log(`  ${testBluetooth ? '' : '不'}测试蓝牙 （代码尚未支持）`)
  console.log(`  wifi：${ssid}`)
  console.log(`  wifi密码：${password}`)

  let digest
  const random = next => fs.lstat('random', (err, stats) => {
    if (err && err.code === 'ENOENT') {
      console.log('生成测试文件用于wifi文件传输测试')
      ...
      if (!stats.isFile() || stats.size !== 1024 * 1024 * 1024) {
        console.log('已有测试文件类型或大小错误，重建测试文件') 
  ...
  random(() => {
    ...
    server.listen(3000, () => console.log('本机（作为文件下载服务器）开始侦听3000端口'))
...
/**
      ...
      ifaces = keys
        .map(k => (ifaces[k].forEach(x => x.name = k), ifaces[k]))
        .filter(x => x.fin
...
*/
    ...
    process.on('SIGINT', () => {
      console.log('')
      console.log('Lord, I\'m fucked up. See you.')
      process.exit(0)
    })
  })
})
