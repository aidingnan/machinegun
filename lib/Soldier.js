...
class StressNg extends EventEmitter {
  constructor (address) {
    ...
    this.child.on('error', err => {
      blog.log(`${this.address}`, '压力测试遇到错误')
...
class WifiTest extends EventEmitter {
  constructor (address, ssid, password, hostAddress) {
    super()
    this.address = address
    this.ssid = ssid
    ...
    this.child = child.exec(`ssh  -o "StrictHostKeyChecking no" -tt root@${this.address} nmcli d wifi connect ${ssid} password ${password}`, (err, stdout) => {
      if (err) {
        blog.log(`${this.address}`, '连接wifi失败')
        // console.log(err)
        this.child = null
        this.error = err
      } else {
        ...
        if (out.includes('successfully activated')) {
          blog.log(`${this.address}`, '设备连接wifi成功')
          const run = () => {
            ...
            let cmd = `ssh -o "StrictHostKeyChecking no" -tt root@${this.address} wget --quiet -O - http://${this.hostAddress}:3000 \\| sha256sum`
            blog.log(`${this.address}`, `第${this.results.length + 1}次wifi下载测试`)
  ...
  constructor (address) {
    ...
    const run = () => {
      ...
      if (this.results.length > 1) {
        cmd = icmd
        n = this.results.length - 2 + 1
      } else {
        cmd = ocmd
        n = this.results.length + 1
      }
      blog.log(`${this.address}`, `第${n}次emmc${this.results.length > 1 ? '读取' : '写入'}测试`)
      this.child = child.exec(cmd, (err, stdout) => {
        if (err) {
          this.child = null
          this.error = err
    ...
    this.child = child.exec(`ssh -o "StrictHostKeyChecking no" -tt root@${this.address} mkdir -p /run/sata`, err => {
      ...
      this.child = child.exec(`ssh -o "StrictHostKeyChecking no" -tt root@${this.address} mount -t btrfs /dev/sda /run/sata`, err => {
        const run = () => {
          ...
          blog.log(`${this.address}`, `第${n}次sata${this.results.length > 1 ? '读取' : '写入'}测试`)
          this.child = child.exec(cmd, (err, stdout) => {
            if (err) {
              this.child = null
              this.error = err
...
class Soldier extends EventEmitter {
  constructor(iface, ssid, password, hostAddress) {
    super()
  ...
  run() {
    ...
    this.wifi = new WifiTest(this.address, this.ssid, this.password, this.hostAddress)
